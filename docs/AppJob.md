...
### AppJob(Path inputDir, SqlReader reader, FileParserProcessor processor, TextWriter writer)
...
- 출력 디렉토리: `D:\11. Project\11. DB\BigQuery_out` (`TextWriter.DEFAULT_OUTPUT_DIR`)
- 출력 문자셋: UTF-8
...
- **소스 테이블**: FROM, JOIN, USING 절에서 참조되는 테이블 목록
...
- Oracle 조인 문법 지원 (FROM table1, table2 형태)
- 한글 테이블명 지원
...
**예외**:
- `IOException` - 파일 쓰기 작업 중 I/O 오류가 발생한 경우
...
기본 설정 대신 특정 경로나 문자셋을 사용하려면 생성자를 직접 호출합니다:
...
- `SqlReader` - 파일 입력 IO 담당
- `FileParserProcessor` - SQL 파싱 담당
- `TextWriter` - 파일 출력 IO 담당
- `TablesInfo` - 테이블 정보 저장 객체

## 설계 철학

이 클래스는 Spring Batch의 설계 철학을 따라 다음과 같은 특징을 가집니다:

1. **관심사의 분리**: Reader, Processor, Writer가 독립적으로 동작
2. **의존성 주입**: 생성자를 통해 각 컴포넌트를 주입받아 유연성 확보
3. **단순성**: 각 STEP은 명확한 단일 책임을 가짐
4. **확장성**: 새로운 Reader, Processor, Writer 구현체로 쉽게 교체 가능
